        ...
        <div class="col-xl-12">
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                    ...
                    @endcan
                    @can('تصدير EXCEL')
                        <a class="modal-effect btn btn-sm btn-primary" href="{{ route('export') }}" style="color:white"><i
                            ...
                            <tbody>
                                @foreach ($invoices as $invoice)
                                    <tr>
                                        ...
                                        <td><a
                                                href="{{ route('invoices_details', $invoice->id) }}">{{ $invoice->sections->section_name }}</a>
                                        </td>
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu tx-13">
                                                    @can('تعديل الفاتورة')
                                                        <a class="dropdown-item"
                                                            href="{{ route('invoices.edit', $invoice->id) }}"><i
                                                                class="text-primary fas fa-edit"></i>&nbsp;&nbsp;تعديل الفاتورة
                                                        </a>
                                                    @endcan
                                                    @can('حذف الفاتورة')
                                                        <button class="dropdown-item">
                                                            <a style="color: black" href="#" class="delete-button"
                                                    ...
                                                    @can('ارشفة الفاتورة')
                                                        <a href="{{ route('archives', $invoice->id) }}" id="archive-link"
...
@section('js')
    ...
    <script>
        document.querySelectorAll('.delete-button').forEach(button => {
            button.addEventListener('click', function(e) {
                ...
                Swal.fire({
                    title: 'هل أنت متأكد؟',
                    text: "هل انت متاكد انك تريد حذف هذه الفاتورة؟",
                    icon: 'warning',
    ...
    <script>
        document.getElementById('archive-link').addEventListener('click', function(e) {
            e.preventDefault(); // لمنع التوجيه الفوري

            const href = this.getAttribute('href'); // الحصول على الرابط من href

            Swal.fire({
                title: 'هل أنت متأكد؟',
                text: "هل انت متأكد من عملية الأرشفة؟",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    // توجه إلى الرابط المخزن في href
                    window.location.href = href;
                    Swal.fire(
                        'تمت الأرشفة!',
                        'تم نقل العنصر إلى الأرشيف.',
                        'success'
                    );
